...
class Plumber {
  ...
  initDomDataSources() {
    for (const pipelineDataSource of this.pipelineModel.DataSources) {
      ...
      if (dataSource.DynamicOut) {
        this.instance.makeSource(domDataSource, this.buildSourceEndpoint(pipelineDataSource.EntityGuid), {
          filter: '.add-endpoint'
        });
      }
    }
  ...
  addEndpoint(domDataSource, endpointName, isIn, pipelineDataSource) {
    ...
    const model = isIn ? this.buildTargetEndpoint(pipelineDataSource.EntityGuid, style) : this.buildSourceEndpoint(pipelineDataSource.EntityGuid, style);
